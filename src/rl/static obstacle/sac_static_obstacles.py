...
class agent():
    ...
    def save_variable(self, i_list, mean_reward, reward_list):
        with open('./result/sac_100/sac_100_2_deep.csv', 'w', newline='') as csvfile:
            # 建立 CSV 檔寫入器
    ...
    def save_(self):
        torch.save(self.actor.state_dict(), './model/sac_100/sac_100_2_deep.pth')

    def save_best(self):
        torch.save(self.actor.state_dict(), './model/sac_100/sac_100_2_best.pth')
...
if __name__ == '__main__':
    ...
    env = environment()
    writer = SummaryWriter('runs/sac_100_deep_2')
    global_step = 0
    for i in range(1000):
        reward_list = []
        b_list.append(i)
